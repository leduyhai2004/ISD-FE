...
const Sidebar = () => {
  ...
  useEffect(() => {
    ...
    else if (path === "/attendance/history") setActiveMenu("attendance-history")
  ...
  return (
    <div className="sidebar">
      ...
      <ul className="menu">
        ...
        {/* Hồ sơ */}
        <li
          className={`menu-item has-dropdown ${showProfileMenu ? "expanded" : ""} ${
            activeMenu === "viewProfile" || activeMenu === "editProfile" ? "active" : ""
          }`}
        >
          <div className="sidebar-dropdown-header" onClick={() => setShowProfileMenu(!showProfileMenu)}>
            <i className="fas fa-user"></i> Hồ sơ
            <span className="dropdown-icon">{showProfileMenu ? "▲" : "▼"}</span>
          </div>
          <ul className={`sidebar-dropdown-menu ${showProfileMenu ? "show" : ""}`}>
            <li
              onClick={() => handleNavigate("/profile/view", "viewProfile")}
              className={`submenu-item ${activeMenu === "viewProfile" ? "active-submenu" : ""}`}
            >
              Xem Hồ sơ
            </li>
            <li
              onClick={() => handleNavigate("/profile/edit", "editProfile")}
              className={`submenu-item ${activeMenu === "editProfile" ? "active-submenu" : ""}`}
            >
              Chỉnh sửa Hồ sơ
            </li>
          </ul>
        ...
        {/* Điểm danh */}
        <li
          className={`menu-item has-dropdown ${showAttendanceMenu ? "expanded" : ""} ${
            activeMenu === "attendance" || activeMenu === "attendance-history" ? "active" : ""
          }`}
        >
          <div className="sidebar-dropdown-header" onClick={() => setShowAttendanceMenu(!showAttendanceMenu)}>
            <i className="fas fa-check-circle"></i> Điểm danh
            <span className="dropdown-icon">{showAttendanceMenu ? "▲" : "▼"}</span>
          </div>
          <ul className={`sidebar-dropdown-menu ${showAttendanceMenu ? "show" : ""}`}>
            <li
              onClick={() => handleNavigate("/attendance", "attendance")}
              className={`submenu-item ${activeMenu === "attendance" ? "active-submenu" : ""}`}
            >
              Check In/Out
            </li>
            <li
              onClick={() => handleNavigate("/attendance/history", "attendance-history")}
              className={`submenu-item ${activeMenu === "attendance-history" ? "active-submenu" : ""}`}
            >
              Lịch Sử Điểm Danh
            </li>
          </ul>
