...
export const DataSyncProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleProfileRequestsUpdate = () => {
      setLastUpdate((prev) => ({ ...prev, profileRequests: Date.now() }));
    };
    ...
    dataEvents.on("profile-requests-updated", handleProfileRequestsUpdate);
    ...
    return () => {
      ...
      dataEvents.off("profile-requests-updated", handleProfileRequestsUpdate);
      clearInterval(checkUserInterval);
    };
  }, [currentUser]);

  return (
    <DataSyncContext.Provider value={{ lastUpdate }}>
      {children}
